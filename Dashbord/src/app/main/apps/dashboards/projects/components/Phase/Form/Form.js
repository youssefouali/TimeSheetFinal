...
const Form = ({ currentId, setCurrentId }) => {
  const [phaseData, setPhaseData] = useState({ creator: '', title: '', description: '', tags: '', selectedFile: '', startDate:'',endDate:'',priority:'',projectId:projectidd,memberid:''});
  ...
  const clear = () => {
    setCurrentId(0);
    setPhaseData({ creator: '', name: '', description: '', tags: '', selectedFile: '', startDate: '', endDate: '' ,priority:'',projectId:projectidd,memberid:''});
  };
  ...
  return (
    <Paper className={classes.paper}>
      ...
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editing "${phase.name}"` : 'Creating a Phase'}</Typography>
        ...
        <select value={phaseData.memberid} onChange={(e) => setPhaseData({ ...phaseData, memberid: e.target.value })}> 
